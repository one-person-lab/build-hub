...
RAW = Path('/tmp/vh_en/raw')
OUT = Path('/tmp/vh_en/data')
...
def parse_catalog(path, page_key, href):
    ...
    for sec in sections:
        ...
        for card in find_all_class(sec, 'card'):
            ...
            # 卡片内的可视化预览（demo），带同名 class 才能复用原站样式
            demo_cls, demo_inner = None, None
            dm = re.search(r'<div class="([^"]*\bcard-demo\b[^"]*)"[^>]*>', card)
            if dm:
                demo_cls = dm.group(1)
                demo_inner = inner_html(card, dm.start())
            terms.append({'slug': data_id, 'name': name, 'en': en,
                          'tagline': text_of(tag_m.group(1)) if tag_m else '',
                          'demoHtml': clean(demo_inner) if demo_inner else '',
                          'demoClass': demo_cls or ''})
...
def parse_term(slug, path):
    ...
    if body:
        for sec in re.split(r'<section', body)[1:]:
            ...
            entry = {'title': title}
            # 结构化 distinctions
            if '容易混淆' in title or 'Confused' in title:
                cards = []
                ...
                entry['cards'] = cards
            elif 'Anatomy' in title or '构造' in title:
                ...
                entry['parts'] = parts
            elif 'Variants' in title or '变体' in title:
                variants = []
            ...
            elif '典型使用场景' in title or 'Scenarios' in title or 'Where' in title:
                scenes = []
                ...
                entry['scenes'] = scenes
            elif '延伸阅读' in title or 'Further' in title or 'Reading' in title:
                refs = []
...
# ---------- run ----------
catalogs = []
for path, key, href in [('index.html', 'frontend', '/en')] + \
        [(f'topics__{k}.html', k, f'/en/topics/{k}') for k in
         ['backend', 'product', 'testing', 'technology', 'ai', 'git', 'design']]:
...
print('total terms:', len(all_slugs))
...
print('terms parsed:', len(terms), 'missing:', len(missing), missing[:10])

special = {
    'practice': 'practice.html',
    'anti-ai-flavor': 'anti-ai-flavor.html',
...
for key, f in special.items():
    ...
    pages[key] = {'title': hm.unescape(title.group(1)) if title else key,
                  'mainHtml': clean(m.group(1)) if m else ''}
